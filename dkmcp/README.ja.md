...
DockMCP は AI と人間の責任を明確に分離しています。AI は観察と提案を担い、人間がインフラ変更を実行します。アクセスは段階的に提供され、各レベルをオプトインで有効化できます。
...
| **ホストアクセス** | ホストツール、ホストコマンド | **無効** | 中〜高 |

上位レベルはデフォルトで無効であり、`dkmcp.yaml` で明示的にオプトインする必要があります。
...
コンテナの再起動はクラッシュからの復旧や設定変更の反映には有効ですが、フルリビルドの代わりにはなりません。DockMCP が `docker-compose build` や `docker-compose up` をサポートしないのは、再起動ですべて解決するという誤解を防ぐためです。
...
コンテナの再起動は、本当に必要な場面（クラッシュからの復旧、環境変数変更の反映）に対応するためオプトインで提供されています。ビルド/再作成は高リスクかつ低頻度のため、人間のみが実行します。

#### 4. AI は調査し、人間がインフラを操作する

**良いワークフロー：**
1. AI がログ、統計、エラーパターンを調査
2. AI が問題を特定し、解決策を提案
3. `lifecycle` が有効なら、AI が単純な復旧としてコンテナを再起動
4. インフラ変更が必要な場合は、**人間が** 判断・実行
...
- **ホストアクセス**（ツール、コマンド）— オプトイン、デフォルト無効
