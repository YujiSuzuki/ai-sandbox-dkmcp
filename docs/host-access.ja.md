...
3 機能とも**デフォルトは無効**です。`dkmcp.yaml` の `host_access` や `security.permissions` で有効化します。
...
`.sandbox/host-tools/` に配置されたスクリプト（`.sh`、`.go`、`.py`）を AI が発見・実行できる機能です。生のシェルコマンドではなく、レビュー済みのスクリプトを通じてホスト側の操作（デモコンテナの起動など）を行えます。
...
- `lifecycle: true` をセキュリティ権限で有効化する必要があります
