...
- **ヘルパースクリプトを自動発見** — `.sandbox/` のユーティリティスクリプトをAIが自動で発見・理解し、ホスト専用の操作には適切なガイダンスを表示
...
**SandboxMCP** がコンテナ内で動作し、AIがユーティリティスクリプトを自動発見できます。「使えるスクリプトは？」と聞けば、説明付きで一覧表示。ホスト専用の操作（コンテナ間の認証情報コピーなど）には、エラーではなく明確な手順を案内します。Claude Code と Gemini CLI の両方に対応し、起動時に自動登録されます。
...
Sandbox とは直接関係ありませんが、`.sandbox/tools/` に Claude Code の会話履歴を検索できるツールが入っています。AIに「昨日の会話で○○について話した？」とお願いすると、過去の会話を検索して答えてくれます。

> 詳しくは [docs/architecture.ja.md](docs/architecture.ja.md) を参照
